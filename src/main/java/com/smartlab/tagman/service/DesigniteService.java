...
public class DesigniteService {
	...
	public List<DesigniteCSV> getdesigniteEntries(String packageName, String className, String projectName) {
		...
		System.out.println("design Entries" + results.size());
		return results;
	}
	...
	public List<DesigniteCSVMethod> getdesigniteMethodEntries(String packageName, String className, String projectName,
		...
		Session session = sessionFactory.getCurrentSession();
		Transaction tx = session.beginTransaction();
		SQLQuery query = session.createSQLQuery("select * from designitecsvdata_method where package_name = \"" + packageName
				+ "\" AND type_name = \"" + className + "\" AND project_name = \"" + projectName + "\" AND method_name = \""+methodName+"\"");
