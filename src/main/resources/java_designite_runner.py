...
def _run_designite_java(folder_name, folder_path, designiteJava_jar_path, smells_results_folder):
    ...
    proc = Popen(["/opt/homebrew/opt/openjdk/bin/java", "-jar", designiteJava_jar_path, "-i", folder_path, "-o", out_folder])
    proc.wait()
...
def _build_project(dir, dir_path):
    ...
    if os.path.exists(pom_path):
        print("Found pom.xml")
        os.chdir(dir_path)
        proc = Popen([r'/opt/homebrew/bin/mvn', 'clean', 'install', '-DskipTests'])
        proc.wait()
        is_compiled = True
    ...
    if os.path.exists(gradle_path):
        ...
        os.chdir(dir_path)
        proc = Popen([r'/opt/homebrew/bin/gradle', 'build','-x','test'])
        proc.wait()
        is_compiled = True
    if not is_compiled:
